# -*- coding: utf-8 -*-
"""
Created on Thu Sep 15 08:30:20 2022
...
class Video(Thread):
    ...
    def floodFill(self, img, r):
        ...
        _, threshold = cv.threshold(image_cropped, 150, 255, cv.THRESH_BINARY)
        ...
        for c in range(len(largest_contours)):
            ...
            img_rect = cv.drawContours(img_copy, [box], 0, (255, 0, 0), offset = (r[0], r[1]), thickness = 2)
            target_x = box_x+int(box_w/3)+r[0]
            target_y = box_y+int(box_h/3)+r[1]
            img_points.append([target_x, target_y])
            cv.circle(img_copy,(box_x+int(box_w/3)+r[0],box_y+int(box_h/3)+r[1]), 3, (0,255,0), -1)
            cv.imshow("Target nr " + str(c), img_copy)
            cv.imwrite("Target nr " + str(c) + ".png", img_copy)
            '''for i in box:
                corners = corners + 1
