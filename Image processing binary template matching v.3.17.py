...
class Robot_TCP_comm(Thread):
        ...
        while True:
            if len(img_points) > 0:
                ...
                for i in range(len(img_points)):
                    ...
                    x_dist_pix, y_dist_pix = self.findDistance(
                        img_points[i][0]+(img_points[i][2]/4), img_points[i][1]+(img_points[i][3]/4))
    ...
    def convertPixeltoMM(self, x_dist_pix, y_dist_pix):
        ...
        x_dist_mm = float((self.camera_distance_2*x_dist_pix *
                          self.sensor_width)/(self.focal_length*1280.0))
        y_dist_mm = float((self.camera_distance_2*y_dist_pix *
                          self.sensor_height)/(self.focal_length*960.0))
...
class Video(Thread):
    def __init__(self, text):
        ...
        self.camera_matrix = np.array(
            [[3.81, 0, 2.018], [0, 3.81, 2.018], [0, 0, 1]])
        self.camera_distance = 864.0
    ...
    def run(self):
        ...
        img, square_flag = self.startVideo("Test")
    ...
    def startVideo(self, run):
        if run == "Run":
            ...
            cap = cv.imread(
                "C:/Users/Viktor From/OneDrive/Kandidat/Kandidatprojekt/02 - Code/Robot control sandbox/sandbox_v.1.5/Viktor From/Original image.PNG")
        square_flag = 0
    ...
    def detectBoxes(self, im):
        ...
        # Smoothing filter to remove noiseq
        im = cv.bilateralFilter(im, 7, 20, 20)
# im = im[800:1800, 800:1800
        gray = cv.cvtColor(im, cv.COLOR_BGR2GRAY)
        cv.imshow("Gray", gray)
        cv.imwrite("Gray.png", gray)
        template = cv.resize(cv.imread(
            "C:/Users/Viktor From/OneDrive/Kandidat/Kandidatprojekt/02 - Code/Robot control sandbox/sandbox_v.1.5/Viktor From/Template Canny image.PNG", 0), (0, 0), fx=0.8, fy=0.8)
        h, w = template.shape
        ...
        Canny = cv.Canny(e_im, 100, 255, True, 7, True) #Canny operation to find lines
        ...
        CannyCrop = Canny[location[1] :location[1]+h, location[0]:location[0]+w]
        ...
        cv.imshow("Cropped Canny image", CannyCrop)
        cv.imwrite("Eroded image.png", e_im)
        contours_list, hierarchy = cv.findContours(
            CannyCrop, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE, hierarchy=1)  # Find contours
        largest_contours = sorted(
            contours_list, key=cv.contourArea, reverse=True)[0:5]
        img_points = []
        for c in range(len(largest_contours)):
            ...
            img_rect = cv.drawContours(
                im, [box], 0, (0, 0, 255), 2, offset=(location[0], location[1]))
            img_rect = cv.drawMarker(im, (location[0]-int(box_x/4), location[1]-int(
                box_y/4)), markerType=cv.MARKER_CROSS, markerSize=20, thickness=1, line_type=8, color=(0, 255, 0))
            ...
            if takePicFlag == 1:
                ...
                # Smoothing filter to remove noise
                im = cv.bilateralFilter(img2, 7, 20, 20)
# im = im[800:1800, 800:1800
                ...
                _, threshold = cv.threshold(gray2, 200, 255, cv.THRESH_BINARY)
                kernel = cv.getStructuringElement(cv.MORPH_ELLIPSE, (3, 3))
                d_im = cv.dilate(threshold, kernel, iterations=2)
        ...
        #imCrop = gray[int(r[1]):int(r[1]+r[3]), int(r[0]):int(r[0]+r[2])]
                for c in range(len(largest_contours)):
                    ...
                    img_rect = cv.drawMarker(img2, (location[0]+int(box_x/4), location[1]+int(
                        box_y/4)), markerType=cv.MARKER_CROSS, markerSize=20, thickness=1, line_type=8, color=(0, 255, 0))
    ...
    def loadIGBTClassifier(self):
        ...
        self.detection_model = tf.keras.models.load_model(
            "C:/Users/Viktor From/OneDrive/Kandidat/Kandidatprojekt/02 - Code/01 - Samples/01 - Machine learning/IGBT_classifier")
